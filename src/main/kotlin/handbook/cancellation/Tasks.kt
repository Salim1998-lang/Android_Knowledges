...
object CancellationTasks {
    ...
    suspend fun <T> orNull(timeoutMs: Long, block: suspend () -> T): T? = TODO()

    /** Л2. Выполнить block с таймаутом; не успел → TimeoutCancellationException. */
    suspend fun <T> strict(timeoutMs: Long, block: suspend () -> T): T = TODO()

    /** Л3. Выполнить block с таймаутом; не успел → вернуть fallback. */
    suspend fun <T> withFallback(timeoutMs: Long, fallback: T, block: suspend () -> T): T = TODO()

    /** Л4. true, если block НЕ уложился в таймаут. */
    suspend fun isTimedOut(timeoutMs: Long, block: suspend () -> Unit): Boolean = TODO()

    /** Л5. Подождать ms (отменяемо) и вернуть value. */
    suspend fun <T> delayThenValue(ms: Long, value: T): T = TODO()

    /** Л6. Кооперативная сумма: перед каждым элементом ensureActive(), затем delay(1) и +=. */
    suspend fun sumCooperatively(values: List<Int>): Int = TODO()

    /** Л7. Построить [0..n-1], уступая поток через yield() между элементами. */
    suspend fun yieldingList(n: Int): List<Int> = TODO()

    /** Л8. Крутить, пока корутина активна: tick(); delay(10). Отмена завершает цикл. */
    suspend fun runUntilCancelled(tick: () -> Unit) { TODO() }
     ...
     */
    suspend fun <T> cooperativeIndexOf(items: List<T>, predicate: suspend (T) -> Boolean): Int? = TODO()
     ...
     */
    suspend fun runChildThenCancel(runMs: Long, tickEvery: Long): Int = TODO()
     ...
     */
    suspend fun sumWithDeadline(timeoutMs: Long, values: List<Int>, stepMs: Long): Int = TODO()
     ...
     */
    suspend fun <T> firstOrSecond(firstMs: Long, first: suspend () -> T, second: suspend () -> T): T = TODO()
     ...
     */
    suspend fun <T> runWithGuaranteedCleanup(block: suspend () -> T, cleanup: suspend () -> Unit): T = TODO()
     ...
     */
    suspend fun <T> raceFirst(a: suspend () -> T, b: suspend () -> T): T = TODO()
     ...
     */
    suspend fun <T> parallelWithDeadline(timeoutMs: Long, blocks: List<suspend () -> T>): List<T>? = TODO()
}
