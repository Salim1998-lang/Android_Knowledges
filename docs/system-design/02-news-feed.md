...
> 🧠 **Два кита, вокруг которых всё крутится:** **плавная прокрутка** (лента забита медиа, а её
> листают бесконечно) и **офлайн** (контент должен открываться без сети). Почти каждое решение ниже
> служит одному из них — держи эту пару в голове, и логика главы соберётся сама.
...
> 🧠 **Правило выбора протокола.** Спроси: *«кто инициирует общение?»* Здесь **всегда клиент** →
> чистый REST. Данные, которые сервер слал бы сам (живые счётчики лайков), потребовали бы WebSocket —
> но real-time мы вынесли за рамки.
...
> 🧠 **Курсор vs offset — решает характер данных.** Offset хорош для **статичных, мало листаемых**
> списков (история заказов, лог операций): дёшево и просто. Но для **живого бесконечного** контента,
> который постоянно пополняется сверху, offset ломается — нужен курсор.
...
> 🧠 **Почему всё на диске, а не в памяти.** Пост не «протухает» за секунду — свежесть некритична, зато
> нужен **офлайн**, а он требует переживать перезапуск. Поэтому единая **реляционная БД на диске**, без
> отдельного RAM-кэша. (Он понадобился бы, будь у данных жёсткое требование к real-time.)
...
> 🧠 **«WebView или нативно?» — решает контекст, а не мода.** Редкий сложный элемент, где нужны мощные
> веб-библиотеки и одинаковый вид на всех платформах (например, финансовый график) → **WebView**.
> Простой контент, который прокручивают без конца → **нативный UI** ради плавности. Один вопрос —
> противоположные ответы в зависимости от того, что рендеришь.
...
**1. Почему для ленты хватает чистого REST, без WebSocket?**

<details><summary>Ответ</summary>

Все взаимодействия — по инициативе клиента (получить ленту, создать пост, лайкнуть): клиент
спрашивает, сервер отвечает — это REST. Данных, которые сервер шлёт *сам* в реальном времени, нет
(обновления и пуши вынесли за рамки). Появись живые счётчики лайков или уведомления о новых постах —
понадобился бы WebSocket. Правило: *«кто инициирует общение?»*
</details>
...
смещения, id уникальны и годятся для сортировки, идеально для «живого» бесконечного контента. (Offset
остаётся ок для статичных, мало листаемых списков — вроде истории заказов.)
</details>
...
**6. Почему текст ленты рендерят нативным UI, а не через WebView?**
...
WebView тяжёлый, «не нативный» и мешает анимации ленты. Он выигрывает в другом случае — редкий
сложный элемент, где важны мощные веб-библиотеки и одинаковый вид на всех платформах. «WebView или
нативно?» — решает контекст того, что рендеришь.
</details>
